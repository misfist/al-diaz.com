...
abstract class CAC_Filtering_Model {
		...
		// Get options for default meta data
			...
			foreach ( $values as $data ) {
				...
				switch ( $field_type ) :
					...
					case "true_false" :
						$empty_option = false;
						if ( 0 == $data->value ) { $field_value = __( 'False', 'codepress-admin-columns' ); }
						if ( 1 == $data->value ) { $field_value = __( 'True', 'codepress-admin-columns' ); }
						break;
					case "page_link" :
					case "post_object" :
	...
	private function display_dropdown( $column_name, $options, $add_empty_option, $current_item = '', $top_label = '' ) { ?>
		<select class="postform cpac_filter" name="cpac_filter[<?php echo $column_name; ?>]" data-current="<?php echo esc_attr( urlencode( $current_item ) ); ?>">
		<?php if ( $top_label ) : ?>
			<option value="">
				<?php echo $top_label; ?>
			</option>
	...
	private function dropdown( $column, $options, $add_empty_option = false, $current_item = '' ) {
		...
		if ( $column->properties->use_filter_operator ) {
			...
			if ( 'between' === $column->options->filter_operator ) {
				$this->display_dropdown( $colum_name, $options, false, $current_item, $top_label . ' - ' . $operators['>='] );
				$this->display_dropdown( $column->properties->name . '-lesser', $options, false, $current_item, $top_label . ' - ' . $operators['<='] );
			}
	...
	public function add_filtering_dropdown() {
		...
		foreach ( $this->storage_model->columns as $column ) {
			...
			if ( ! apply_filters( 'cac/is_cache_enabled', true )  ) {
				$this->delete_dropdown_cache( $column );
			}
	 ...
	 * @since 3.6
	 */
	private function delete_dropdown_cache( $column ) {
		delete_transient( $this->get_cache_id( $column->get_name() ) );
	}
	 ...
	 * @since 3.6
	 */
	public function is_timeout_expired( $id = 'acfilterctimer' ) {
		$cache_timer = $id . $this->storage_model->key;
		if ( get_transient( $cache_timer ) ) {
			return false;
		}
		set_transient( $cache_timer, true, 60 ); // 1 minute
		return true;
	}
	...
	public function ajax_update_dropdown_cache() {
		...
		if ( ! $this->is_timeout_expired() && apply_filters( 'cac/is_cache_enabled', true ) ) {
			wp_send_json_error();
		}
		...
		foreach ( (array) $this->storage_model->columns as $column ) {
			if ( $this->is_filterable( $column ) ) {
				...
				if ( $dropdown_options = $this->get_cached_dropdown_options( $column ) ) {
